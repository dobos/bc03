...
!	Select quantity to compute
	...
	m=0
	l=0
!	write (6,*) 'Using filter:',i,k,pos_i(k),pos_f(k),x(1),x(n),n
...
!		Compute number of photons, instead of total flux (Fukugita et al. 1996)
	...
	elseif (ik.eq.1) then
!		Compute filter = number of photons through filter
		xf(m)=xlam(j)
...
!	Add wavelength points in the sed.
	l=0
	do k=1,n
	xz=x(k)/z1
...
!	Computes effective flux seen through the Ith filter at redshift Z (monochormatic flux)
	...
	INCLUDE 'filter.dec'
	real x(n),y(n),a(20000)
	data a/20000*1./
	aflux=filter_n(i,x,y,n,z,kerr)
	area =filter_n(i,x,a,n,z,kerr)
